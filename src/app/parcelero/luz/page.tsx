...
export default async function ParceleroLuzPage() {
  ...
  const [{ data: cuentas }, { data: lecturas }, { data: facturas }, { data: consumoHistorico }] = await Promise.all([
    ...
    supabase
      ...
      .select('id, mes, anio, archivo_factura_url')
  ...
  type Factura = { id: string; mes: number; anio: number; archivo_factura_url: string }
  ...
  const facturasOrd = (facturas ?? []) as Factura[]
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-1">⚡ Cuenta de luz — Parcela #{parcela.numero}</h1>
                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      <p className="text-sm text-gray-500 mb-1">
                        Período: {nombrePeriodoConRango(c.periodo.mes, c.periodo.anio)}
                      </p>
          ...
          <tbody>
            {lecturasOrd.map(l => (
              <tr key={l.id} className="border-t">
                <td className="px-4 py-2">{nombrePeriodoConRango(l.periodo.mes, l.periodo.anio)}</td>
            ...
            {cuentasOrd.map(c => {
              ...
              return (
                <tr key={c.id} className="border-t">
                  <td className="px-4 py-2">{nombrePeriodoConRango(c.periodo.mes, c.periodo.anio)}</td>
          ...
          <tbody>
            {facturasOrd.map(f => (
              <tr key={f.id} className="border-t">
                ...
                <td className="px-4 py-2"><a href={f.archivo_factura_url} target="_blank" rel="noreferrer" className="text-blue-600 hover:underline">📎 Ver / descargar</a></td>
              </tr>
            ))}
            {facturasOrd.length === 0 && (
