...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { count: cajaExistente } = await supabase
    .from('caja_movimientos')
    .select('id', { count: 'exact', head: true })
    .eq('pago_id', pago.id)
  if (accion === 'validar' && pago.monto && pago.monto > 0 && !cajaExistente) {
  ...
  await registrar(sesion, accion === 'validar' ? 'validar_pago' : 'rechazar_pago', 'pago', id, { cuenta_id: pago.cuenta_id, registrado_en_caja: accion === 'validar', motivo: accion === 'rechazar' ? motivo : undefined })
